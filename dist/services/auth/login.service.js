...
const loginService = (body) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const isPasswordValid = yield (0, argon2_1.comparePassword)(password, user.password);
        if (!isPasswordValid) {
            throw new Error("Incorrect password");
        }
        const { password: pass } = user, userWithoutPassword = __rest(user, ["password"]);
        const token = (0, jsonwebtoken_1.sign)({ id: user.id, role: user.role }, config_1.JWT_SECRET, { expiresIn: "2h" });
